...
int main() {
    ...
    program = compile_program_from_file("madd_int_kernel.cl", context, &err);
    ...
    d_A = clCreateBuffer(
        context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR, sizeof(A), A, &err
    );
    check_error(err, "creating device buffer A");

    d_B = clCreateBuffer(
        context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR, sizeof(B), B, &err
    );
    check_error(err, "creating device buffer B");

    d_C = clCreateBuffer(
        context, CL_MEM_WRITE_ONLY | CL_MEM_COPY_HOST_PTR, sizeof(C), C, &err
    );
    ...
    err = clEnqueueReadBuffer(
        queue, d_C, CL_TRUE, 0, sizeof(C), C, 0, NULL, NULL
    );
...
bool check_matrices_equal(
    const int A[ORDER][ORDER], const int B[ORDER][ORDER]
) {
